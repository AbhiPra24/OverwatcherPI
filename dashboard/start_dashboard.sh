...
nohup streamlit run "$SCRIPT_DIR/app.py" --server.address 127.0.0.1 --server.port 8501 --server.headless true > "$SCRIPT_DIR/dashboard.log" 2>&1 &
PID=$!
echo $PID > "$SCRIPT_DIR/dashboard.pid"

LAN_IP=$(hostname -I | awk '{print $1}')
echo "Dashboard started in background (PID $PID)."
echo "Access it locally at: http://127.0.0.1:8501 (Use Caddy for remote access)"
